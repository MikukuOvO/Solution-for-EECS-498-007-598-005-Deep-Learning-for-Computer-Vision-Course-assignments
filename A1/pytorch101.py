...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    st = (start - 1) // 10 * 10 + 10
    ed = (stop - 1) // 10 * 10+ 10
    x = torch.arange(st, ed, step=10, dtype=torch.float64)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean_x = torch.sum(x, dim=0) / x.shape[0]
    std_x = torch.sqrt(((x - mean_x) ** 2).sum(dim=0) / (x.shape[0] - 1))
    y = x.clone()
    y -= mean_x
    y /= std_x
    ##########################################################################
