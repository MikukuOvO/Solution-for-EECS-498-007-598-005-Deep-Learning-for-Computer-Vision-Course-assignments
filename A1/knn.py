...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x2_train = torch.sum(x_train_flat.pow(2), dim=1).view(-1, 1)
    x2_test = torch.sum(x_test_flat.pow(2), dim=1).view(-1, 1)

    dists = (x2_train + x2_test.t()) - 2.0 * (torch.mm(x_train_flat, x_test_flat.t()))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    val, idx = torch.topk(dists, k, dim=0, largest=False)
    labels = y_train[idx]
...
def knn_cross_validate(
    ...
    # Replace "pass" statement with your code
    num = x_train.shape[0]
    x_train_flat = x_train.view(num, -1)
    x_train_folds = torch.chunk(x_train_flat, chunks=num_folds)
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            val_set_x = x_train_folds[i]
            train_set_x = torch.cat([x_train_folds[j] for j in tmp])

            val_set_y = y_train_folds[i]
            train_set_y = torch.cat([y_train_folds[j] for j in tmp])

            classifier = KnnClassifier(train_set_x, train_set_y)
            acc = classifier.check_accuracy(val_set_x, val_set_y, k = k, quiet=True)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    best_acc = 0
    for k, acc_lists in k_to_accuracies.items():
        cur_acc = torch.sum(torch.tensor(acc_lists)) / len(acc_lists)
        if cur_acc > best_acc:
            best_acc = cur_acc
            best_k = k
    ##########################################################################
